...
export default function Account4LogOut(props) {
    ...
    return (
        ...
        {showSignOutToast &&<CustomToast message="Logout successful!" isError={false}/>}
        {showSignOutToastError && <CustomToast message="Logout failed!" isError={true}/>}
          ...
          <button className="logoutSec-btn" onClick={signOutFunc}>Logout</button>
        </div>
        </>
    )
}