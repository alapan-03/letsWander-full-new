...
//   console.log(me.name)
  ...
  return (
    <div className="dash2-cont">
      {image && isPopupOpen && (
        <Popup className="pfp-prev-cont" open={isPopupOpen} onClose={() => setIsPopupOpen(false)} position="right center" contentStyle={popupStyles}>
          <div className="pfp-prev">
            <img className="preview-pic" src={URL.createObjectURL(image)} alt="Profile Preview" />
          </div>

          <button className="dash-update-btn" onClick={handleUpload}>{clickedUpload ? <Loader/> : "Upload"}</button>
        </Popup>
      )}
        ...
        <div className="dash2-i1">
          <div className="dash2-account">
            ...
            <div className="dash2-photo-cont">
              {uploadPhoto ? <img className="profile-photo" src={uploadPhoto} alt="Profile" /> : 
              userPhoto ? <img className="profile-photo" src={userPhoto} alt="Profile" /> : 
              cookies.get("photo") ? (
                <img className="profile-photo" src={cookies.get("photo")} alt="Profile" />
              ) : (
            ...
            <button className="dash-update-btn" onClick={handleUpdate}>Update</button>
          </div>
        </div>
      </div>
    </div>
